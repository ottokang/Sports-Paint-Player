...
var pathMask = {
    ...
    _drawCircleMask(e, scale = 1) {
        ...
        ctx.beginPath();
        ctx.arc(e.offsetX, e.offsetY, $("#path_mask_radius").val() * scale, 0, 2 * Math.PI);
        ctx.closePath();
        ctx.fill();
        ctx.globalCompositeOperation = "source-over";
    },
    ...
    _drawPathMask(e) {
        let pathMaskCoordinates1 = this._getPathMaskCoordinates(this._x, this._y, e.offsetX, e.offsetY, Number($("#path_mask_radius").val()));
        let pathMaskCoordinates2 = this._getPathMaskCoordinates(
            e.offsetX,
            e.offsetY,
            this._x,
            this._y,
            $("#path_mask_radius").val() * $("#path_mask_scale").val()
        );
        ctx.globalCompositeOperation = "destination-out";
